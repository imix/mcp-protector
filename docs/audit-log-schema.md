...
The current schema version is **1**.  The `version` field will be incremented
...
```json
{"version":1,"timestamp":"2026-02-19T16:00:01.123Z","event":"tool_call","session_id":"42","upstream":"my-server","tool_name":"delete_all","allowed":false}
```
...
- The `session_id` counter starts at 1 and increments atomically per process
  invocation.  It is not persisted across restarts.
